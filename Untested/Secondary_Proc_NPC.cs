...
{
	[NPCGuildScript("Secondary Proc NPC")]
    public class SecondaryProcNPC : GameNPC
	{
        private string PROCNPC_ITEM_WEAK = "DOL.GS.Scripts.SecondaryProcNPC_Item_Manipulation";//used to store the item in the player
		...
		public override bool Interact(GamePlayer player)
		{
			if(base.Interact(player))
			{
				...
				SendReply(player, 	"Greetings Adventurer!\n\n"+
				          			...
				          			"It costs only 500 bp (Bounty Points).\n\n"+
		...
		public override bool ReceiveItem(GameLiving source, InventoryItem item)
		{
			...
			if (item.Object_Type > 1 &&  item.Object_Type < 27) //Item is a weapon
                SendReply(t, "Alright, now select the type of proc you would like to add:\n" +
								"[Direct Damage]\n"+
								"[Damage over Time]\n"+
								"[Damage Add]\n"+
								"[Lifetap]\n"+
